...
pub fn run_validation(
    ...
    for validation_command in validation_commands {
        let capture_outcome =
            capture_command(build_capture_request(&repo_root, &validation_command))?;
    ...
    if !matches!(final_status, ValidationRunStatus::Fail) {
        final_status = if has_optional_failure {
...
fn build_capture_request(repo: &Path, command: &ValidationCommand) -> CaptureRequest {
    CaptureRequest {
        id: command.id.clone(),
